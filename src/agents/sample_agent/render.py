...
class TagNode:
    ...
    def to_html(self, text_map: dict, indent: int = 0) -> str:
        ...
        if not self.children:
            # 자식 없는 경우: 태그와 텍스트를 한 줄로
            return f"{indent_str}<{self.tag}>{node_text}</{self.tag}>"
        else:
            # 자식 있는 경우: 자식 태그들만 포함
            lines = []
...
class TagContent(BaseModel):
    ...
    # 태그별 텍스트 길이 제한 정의
    TAG_LENGTH_LIMITS: ClassVar[Dict[str, int]] = {
        "h1": 20,
        "h2": 30,
        "h3": 40,
        "p": 100,
    ...
    def validate_text_length(cls, v: str, info) -> str:
        tag = info.data.get("tag", "").lower()
        limit = cls.TAG_LENGTH_LIMITS.get(tag, cls.TAG_LENGTH_LIMITS["default"])

        real_limit = limit + 10
        if len(v) > real_limit:
            # TODO: 에러에 따른 처리 방법 추가
            raise ValueError(
                f"<{tag}> 태그의 텍스트는 최대 {real_limit}자까지 허용됩니다. "
                f"현재 길이: {len(v)}자"
            )
        return v

    @field_validator("text")
    def validate_parent_text(cls, v: str, info) -> str:
        # 자식이 있는 경우 텍스트를 허용하지 않음
        if info.data.get("children") and v:
            raise ValueError(
                f"자식 태그가 있는 <{info.data.get('tag')}> 태그에는 "
                "직접적인 텍스트를 포함할 수 없습니다."
            )
        return v
    ...
    class Config:
        json_schema_extra = {
            "examples": [
                {"tag": "h1", "text": "제목", "children": []},
                {
                    "tag": "div",
                    "text": "컨테이너 텍스트",
                    "children": [{"tag": "p", "text": "단락 텍스트", "children": []}],
                },
            ]
        }
...
웹빌더를 랜딩페이지에 들어가는 페이지에 넣어야 하는 텍스트를 생성해 주세요.

에멧 코드: {emmet_code}
""".strip()

    # 3) Chat 모델에 structured output 스키마 바인딩 (즉, 직접 Pydantic 객체를 반환)
    ...
    # 5) 반환받은 tagContents를 기존 TagNode 트리와 매핑하여 HTML 텍스트 생성
